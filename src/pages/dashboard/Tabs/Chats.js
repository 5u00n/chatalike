...
class Chats extends Component {
  ...
  openUserChat(e, chat) {
    ...
    var recentChatList = this.props.recentChatList;

    var index = -1;
    if (!recentChatList || recentChatList?.length === 0) {
      index = -1; // or handle the error as needed
      console.error("recentChatList is null, undefined, or empty");
    } else {
      index = recentChatList.indexOf(chat);
    }
          ...
          <div>
            ...
            <SimpleBar className="chat-message-list">
              <ul className="list-unstyled chat-list chat-user-list px-2" id="chat-list">
                {(this.state.recentChatList ) &&
                  Object.values(this.state?.recentChatList).map((chat, key) => {
                            ...
                            {chat.profilePicture === "Null" ? (
                              <div className={"chat-user-img " + chat.status + " align-self-center me-1 ms-0"}>
                                <div className="avatar-xs">
                                  <span className="avatar-title rounded-circle bg-primary-subtle text-primary">{chat.name.charAt(0)}</span>
                                </div>
                                {chat.status && <span className="user-status"></span>}
                              </div>
                            ) : (
                              <div className={"chat-user-img " + chat.status + " align-self-center me-1 ms-0"}>
                                <img src={chat.profilePicture} className="rounded-circle avatar-xs" alt="ChatAlike" />
                                {chat.status && <span className="user-status"></span>}
                              </div>
                            )}
                            ...
                            <div className="flex-grow-1 overflow-hidden">
                              ...
                              <p className="chat-user-message font-size-14 text-truncate mb-0 ms-3">
                                {chat.isTyping ? (
                                  ...
                                  <>
                                    {chat.messages && chat.messages.length > 0 && chat.messages[chat.messages.length - 1].isImageMessage === true ? (
                                      <i className="ri-image-fill align-middle me-1"></i>
                                    ) : null}
                                    {chat.messages && chat.messages.length > 0 && chat.messages[chat.messages.length - 1].isFileMessage === true ? (
