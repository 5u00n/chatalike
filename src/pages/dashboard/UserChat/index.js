...
//actions
import { openUserSidebar, setFullUser, fetchDataRequest } from "../../../redux/actions";
...
function UserChat(props) {
  ...
  useEffect(() => {
    ...
    //props.setFullUser(null, true);
    if (ref.current.el) {
      ref.current.getScrollElement().scrollTop = ref.current.getScrollElement().scrollHeight;
    }
  }, [props.active_user, props.recentChatList, props]);
  ...
  const addMessage = (message, type) => {
    ...
    props.setFullUser(copyallUsers, true);
    ...
    <React.Fragment>
      <div className="user-chat w-100 overflow-hidden">
        <div className="d-lg-flex">
                        ...
                        <div className="conversation-list">
                          <div className="chat-avatar">
                            {chat.userType === "sender" ? (
                              <img src={avatar1} alt="ChatAlike" />
                            ) : props.recentChatList[props.active_user].profilePicture === "Null" ? (
                              <div className="chat-user-img align-self-center me-3">
                                <div className="avatar-xs">
                                  <span className="avatar-title rounded-circle bg-primary-subtle text-primary">{chat.userName && chat.userName.charAt(0)}</span>
                                </div>
                              </div>
                            ) : (
                              <img src={props.recentChatList[props.active_user].profilePicture} alt="ChatAlike" />
                            )}
                          </div>
                              ...
                              <div className="ctext-wrap-content">
                                ...
                                {!chat.isTyping && (
                                  <p className="chat-time mb-0">
                                    <i className="ri-time-line align-middle"></i> <span className="align-middle">{chat.time}</span>
                                  </p>
                                )}
                              </div>
                              {!chat.isTyping && (
                              ...
                              chatMessages[key].userType === chatMessages[key + 1].userType ? (
                                ...
                                <div className="chat-avatar">
                                  {chat.userType === "sender" ? (
                                    ...
                                    <div className="chat-user-img align-self-center me-3">
                                      <div className="avatar-xs">
                                        <span className="avatar-title rounded-circle bg-primary-subtle text-primary">{props.recentChatList[props.active_user].name.charAt(0)}</span>
                                      </div>
                                    </div>
                                  ) : (
                                    <img src={props.recentChatList[props.active_user].profilePicture} alt="ChatAlike" />
                          ...
                          <div className="user-chat-content">
                            ...
                            {chatMessages[key + 1] ? (
                              chatMessages[key].userType === chatMessages[key + 1].userType ? null : (
                                <div className="conversation-name">{chat.userType === "sender" ? "Patricia Smith" : props.recentChatList[props.active_user].name}</div>
                              )
                            ) : (
          ...
          <UserProfileSidebar activeUser={props?.recentChatList ? props.recentChatList[props.active_user] : null} />
        </div>
      </div>
    </React.Fragment>
  );
